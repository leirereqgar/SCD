...
int cigarrillos[4] = {0,0,0,0};
Semaphore mostrador = 1;
Semaphore ingredientes[4] = {0,0,0,0}; //Controlar si los fumadores tienen su ingrediente o no
mutex mtx;
...
void  funcion_hebra_fumador(int num_fumador) {
	while(true) {
		...
		if (cigarrillos[num_fumador]%2 == 0) {
			mtx.lock();
			cout << "\tEs par" << endl;
			mtx.unlock();
			fumar(num_fumador);
...
void inicializarHebras (vector<thread> fumadores, const int num_fumadores) {
...
int main() {
	...
	inicializarHebras(fumadores, NUM_FUMADORES);
